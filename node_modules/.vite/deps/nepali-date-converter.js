//#region node_modules/nepali-date-converter/dist/nepali-date-converter.es5.js
/******************************************************************************
...
*/
var MAX_DAY = 33238;
if (daysPassed !== MAX_DAY) throw new Error("Invalid constant initialization for Nepali Date.");
/**
* Min possible Day
*/
var MIN_DAY = 1;
...
function findPassedDays(year, month, date) {
	try {
		...
		var daysPassed_1 = pastYearDays + (yearDaysMapping[yearIndex + extraYear][COMPLETED_DAYS] - pastYearDays + monthDaysMappings[yearIndex + extraYear][extraMonth][COMPLETED_DAYS]) + date;
		if (daysPassed_1 < MIN_DAY || daysPassed_1 > MAX_DAY) throw new Error();
		return daysPassed_1;
	} catch (_a) {
...
function mapDaysToDate(daysPassed) {
	if (daysPassed < MIN_DAY || daysPassed > MAX_DAY) throw new Error("The epoch difference is not within the boundaries ".concat(MIN_DAY, " - ").concat(MAX_DAY));
...
function format(bsDate, stringFormat, language) {
	return stringFormat.replace(/((\\[MDYd])|D{1,2}|M{1,4}|Y{2,4}|d{1,3})/g, function(match, _, matchedString) {
		var _a;
		switch (match) {
			case "D": return mapLanguageNumber(bsDate.date.toString(), language);
			case "DD": return mapLanguageNumber(bsDate.date.toString().padStart(2, "0"), language);
			case "M": return mapLanguageNumber((bsDate.month + 1).toString(), language);
			case "MM": return mapLanguageNumber((bsDate.month + 1).toString().padStart(2, "0"), language);
			case "MMM": return formatObj[language].month.short[bsDate.month];
			case "MMMM": return formatObj[language].month.long[bsDate.month];
			case "YY": return mapLanguageNumber(bsDate.year.toString().slice(-2), language);
			case "YYY": return mapLanguageNumber(bsDate.year.toString().slice(-3), language);
			case "YYYY": return mapLanguageNumber(bsDate.year.toString(), language);
			case "d": return mapLanguageNumber(((_a = bsDate.day) === null || _a === void 0 ? void 0 : _a.toString()) || "0", language);
			case "dd": return formatObj[language].day.short[bsDate.day || 0];
			case "ddd": return formatObj[language].day.long[bsDate.day || 0];
			default: return matchedString.replace("/", "");
		}
	}).replace(/\\/g, "");
}
function parse(dateString) {
	...
	var match = dateString.match(OFFICIAL_FORMAT);
...
var NepaliDate = function() {
	function NepaliDate() {
		var constructorError = /* @__PURE__ */ new Error("Invalid constructor arguments");
		if (arguments.length === 0) this[convertToBSMethod](/* @__PURE__ */ new Date());
			...
			switch (typeof argument) {
				...
				case "string":
					...
					break;
				case "object":
					if (argument instanceof Date) this[convertToBSMethod](argument);
					else throw constructorError;
					break;
				default: throw constructorError;
			}
